...
class Weather(commands.Cog):
    ...
    async def get_weather(self, ctx, *, city: str = None):  
        ...
        # Sadece belirli bir kanalda çalışacak şekilde kontrol et
        if ctx.channel.id != 1340760164617424938:
            embed = discord.Embed(
                title="Hrrrr!",
                description=f"Lütfen <#{1340760164617424938}> kanalında buluşalım. Kediler burada mutlu! 😸",
        ...
        if not city:
            await ctx.send("❌ Lütfen bir şehir belirtin! Örnek: `!h İstanbul`")
            return
